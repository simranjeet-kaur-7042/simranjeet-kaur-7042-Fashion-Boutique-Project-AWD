document.getElementById("consultForm").addEventListener("submit", async function(e){
    ...
    // POST to Fake API (updated database name)
    try {
        const res = await fetch("http://localhost:3000/consultations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ fullName, email, phone, budget, occasion, stylePref, size })
        });

        if(res.ok){
            responseMsg.textContent = "Consultation request submitted successfully!";
        ...
        console.log(err);
        responseMsg.textContent = "Server error! Make sure JSON server is running.";
        responseMsg.style.color = "red";
    }
});
